...
void BME280_initialize()
{
    ...
    cmd[0] = 0x8E; // i2c_read dig_P regs
    i2c_write(cmd, 1);
    i2c_read(cmd, 18);
...
void i2c_write (uint8_t *pData, uint16_t Size){
	HAL_I2C_Master_Transmit(_BME_hi2c, address, pData,Size, TIMEOUT_I2C);

}

void i2c_read (uint8_t *pData, uint16_t Size){
	HAL_I2C_Master_Receive(_BME_hi2c, (address+1), pData, Size, TIMEOUT_I2C);
